...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let cloud_host = if args.len() > 1 { args[1].as_str() } else { "127.0.0.1" };
        ...
        match notification {
          Event::Incoming(Incoming::Publish(publish)) => {
            if let Ok(payload) = std::str::from_utf8(&publish.payload) {
              ...
              match serde_json::from_str::<pozor_dom_shared::dashboard::lib::DeviceTelemetry>(
                payload,
              ) {
                Ok(telemetry) => {
...
async fn start_web_server_with_db(
  ...
  let dashboard = warp::path::end().map(move || {
  ...
  let api_devices =
    warp::path!("api" / "devices").and(db_filter.clone()).map(|db: Arc<database::Database>| {
      // Hub always has access to its database
      match db.load_devices() {
        Ok(devices) => {
          let device_list: Vec<&pozor_dom_shared::dashboard::lib::DeviceTelemetry> =
        ...
        Err(e) => {
          ...
          warp::reply::with_status(
            warp::reply::json(&Vec::<&pozor_dom_shared::dashboard::lib::DeviceTelemetry>::new()),
  ...
  let api_messages =
    warp::path!("api" / "messages").and(hub_state_filter.clone()).and_then(get_messages);
